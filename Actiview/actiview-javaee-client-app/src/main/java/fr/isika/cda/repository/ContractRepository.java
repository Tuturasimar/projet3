...
public class ContractRepository {
	...
	public Long register(ContractCreationViewModel contractVM) {
		...
		List<Options> options = findAllOptions();
		for (Options option : options) {
			...
			entityManager.persist(contractOptions);
		}
	...
	public List<Options> findOptions(long id) {
		try {
			return entityManager.createQuery(
					"SELECT o FROM ContractOptions opt JOIN opt.option o WHERE opt.contract.id = :contractIdParam",
					Options.class).setParameter("contractIdParam", id).getResultList();
	...
	public List<Options> findAllOptions() {
		try {
			return entityManager.createQuery("SELECT o FROM Options o", Options.class)
					.getResultList();
		} catch (NoResultException e) {
			return null;
		}
	...
	public Feature findFeature(Long idFeature) {
		...
		try {
			return entityManager
					...
					.setParameter("id", idFeature).getSingleResult();
		} catch (NoResultException e) {
			return null;
		}
