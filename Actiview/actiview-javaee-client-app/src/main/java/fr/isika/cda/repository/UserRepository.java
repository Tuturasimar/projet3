...
public class UserRepository {
	...
	public User findUserWithManagerById(Long id) {
		Query query = em.createQuery(""
				+ "SELECT u FROM User u "
				+ "LEFT JOIN FETCH u.manager WHERE u.id = :id");
		query.setParameter("id", id);
		return (User) query.getSingleResult();
	}
	...
	public List<User> getAllEmployeesByManagerId(Long managerId) {
		Query query = em.createQuery(""
				+ "SELECT u FROM User u "
				+ "JOIN u.userData LEFT JOIN FETCH u.manager m "
				+ "WHERE m.id = :id")
	...
	public User checkIfUserExists(LoginViewModel loginVm) {
		return em
				.createQuery(""
						+ "SELECT u FROM User u "
						+ "WHERE u.login = :loginParam AND u.password = :passwordParam",
						User.class)
	...
	public Company findCompanyByUserConnected() {
		Query query = em.createQuery(""
				+ "SELECT u.company FROM User u "
				+ "WHERE u.id = :userId", Company.class);
